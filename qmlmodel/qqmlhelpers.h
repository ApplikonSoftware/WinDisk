...
#define QML_PROPERTY_OBJECT(name) \
    protected: \
    Q_PROPERTY (QObject* name READ get_##name NOTIFY name##Changed) \
    private: \
    QObject* m_##name; \
    public: \
    QObject* get_##name () const { \
        return m_##name ; \
    } \
    bool update_##name (QObject* name) { \
    ...
    Q_SIGNALS: \
    void name##Changed (QObject* name); \
    private:
